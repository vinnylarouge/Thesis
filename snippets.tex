...
And we have replaced commutation with natural isomorphisms $\theta$ that are depicted as twisting wires in string diagrams. Recalling that we're happy with isomorphism in place of equality, we would like to know that every possible composite of these structural operations is an isomorphism.
}

\marginnote{
\begin{remark}[Coherence for symmetric monoidal categories]
Going back to addition, we can rearrange sums by commutativity of the addition monoid: . In the monoidal setting, the natural isomorphism $\theta$ is the twisting of wires. Just as we would like to have addition equations such as $(x + (y + z)) = ((z + y) + x)$, we would like the twists $\theta$ to behave well with respect to the bundling of objects and morphisms with the associator $\alpha$.
\end{remark}
}


The introduction rules for verbs, adjectives, modifiers, \emph{unsaturated} nouns (dotted) and \emph{saturated} nouns (solid).


Going forward we'll just call the above a 1-object-4-category since there is a single 0-cell object, and the highest dimension we consider is 4. Symmetric monoidal categories are equivalently seen as 1-object-4-categories \bR CITE \e, which are in particular obtained by suspending 1-object-2-categories for planar string diagrams.


\begin{defn}[Sentence structure]\label{dfn:sentCSG}
\end{defn}
A sentence can be (1) a simple sentence, which may (2) generate unsaturated nouns from the right. (3) a pair of sentences with a conjunction in between. (4) if there is a single unsaturated noun, it may become a sentence with a sentential-complement verb that scopes over another sentence.
The corresponding CSG rules are depicted.


The left text circuit may be read \texttt{C hates D. A likes B.}, and the right as \texttt{A likes B. C hates D.} The middle text circuit can be rewritten as either the left or right, which can be done if we're happy to make such choices. A choiceless approach requires
...
Passing through the split idempotent, we can recast each as a circuit gate using copy maps.
