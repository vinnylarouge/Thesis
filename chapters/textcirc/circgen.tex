...
For the lexicon $\mathcal{L}$, the \emph{text grammar} $\mathcal{T}$ is the set of strings of $\mathcal{L}^\texttt{*}$ associated with finished text diagrams. The sentence-wise grammaticality is gauged by Propositions \ref{prop:simpsent} and \ref{prop:compsent}.
\end{defn}
