...
\begin{convention}[Contentless conjunctions]
\end{convention}
Conventions \ref{conv:sliding} and \ref{conv:reading} require something else to allow them to work at the same time. The left text circuit may be read \texttt{C hates D. A likes B.}, and the right as \texttt{A likes B. C hates D.} The middle text circuit requires something to distinguish the fact that the gates are parallel: a "contentless" conjunction, such as \texttt{and}, or \texttt{while}. In terms of text diagram rewrites, we want something like this:
...
\]
In process theories, wires are identity processes that do nothing to their inputs. We require a text-diagram analogue, and an intransitive "null-verb" in English that seems to work is \texttt{is}, in the sense of \texttt{exists}. In terms of text diagram rewrites, we want something like this:
\end{convention}
