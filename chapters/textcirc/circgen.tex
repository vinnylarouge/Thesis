...
\centering
\[
\resizebox{0.6\textwidth}{!}{\tikzfig{mushroom/simpleADP}}
\]
\caption{
...
\begin{proof}
By graphical correspondence; viewing nodes on the pink surface as 1-cells, each rewrite rule yields a 2-cell. Reading top-down, the dashed-blue helper lines for adpositions indicate \textcolor{blue}{\texttt{ADP}}-pass rules. The correspondence is visually evident, but may be expressed as a separate signature. The correspondence \textcolor{green}{\texttt{IV}}-intro is depicted, to be read from left-to-right.
\end{proof}
\end{proposition}
}
\end{figure}
...
\centering
\[
\resizebox{\textwidth}{!}{\tikzfig{mushroom/complexsentenceCSG}}
\]
\caption{
\begin{proposition}\label{prop:compsent}
Up to labels, Rules \ref{rules:simp} and \ref{rules:comp} for simple and complex sentence yield the same sentences as the combined CSG of Construction \ref{dfn:simpCSG} with the depicted additional rules.
\begin{proof}
...
\centering
\[
\resizebox{0.9\textwidth}{!}{\tikzfig{mushroom/circuitplan}}
\]
...
\centering
\[
\resizebox{0.9\textwidth}{!}{\tikzfig{mushroom/reflcomplication}}
\]
\caption{Reflexive coreference would violate of the processivity condition of string diagrams for symmetric monoidal categories. Not all symmetric monoidal categories possess the appropriate structure to interpret such reflexive pronouns, but there exist interpretative options. From left to right in roughly decreasing stringency, compact closed categories are the most direct solution. More weakly, traced symmetric monoidal categories also suffice. If there are no traces, so long as the noun wire possesses a monoid and comonoid, a convolution works. If all else fails, one can just specify a new gate. We provide a purely syntactic treatment in \bR CITE \e; for now we treat them as if they were just verbs of lower arity.}
\end{figure}
...
\]
\caption{
\begin{term}[Kinds of nouns with respect to coreference]
...
\centering
\[
\resizebox{\textwidth}{!}{\tikzfig{textcirc/gateeqslide}} 
\]
\caption{
...
Conventions \ref{conv:sliding} and \ref{conv:reading} require something else to allow them to work at the same time: something to distinguish when the gates are parallel. So we ask for "contentless" conjunctions, such as \texttt{and}, or \texttt{while}. In terms of text diagrams, we want rewrites that introduces such contentless conjunctions and witnesses their associativity, like this:
\end{convention}
}
\end{figure}
