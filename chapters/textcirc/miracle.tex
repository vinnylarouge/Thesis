...
\newthought{There is a mathematical issue:} the "filling in" of internal wirings is not \emph{in general} functorial, for either speaker or listener. The exemplified issue is that sometimes the particular internal wiring depends on what words are around it.
...
\begin{defn}[Monoidal discrete opfibration]\label{defn:mondiscopf}
...
(Presentation from \citep{wilsonStringDiagramsNonstrict2022a}) Fix an arbitrary (non-strict) monoidal category $\mathcal{C}$. The \emph{strictification} $(\overline{\mathcal{C}},\bullet)$ is defined as follows (where strictness of $\overline{\mathcal{C}}$ entitles use of string-diagrammatic notation):
\begin{itemize}
...
\marginnote{
\begin{proposition}[$\bar{\mathcal{M}}$ and $\mathcal{M}$ are monoidally equivalent]\label{prop:strictequiv}\citep{wilsonStringDiagramsNonstrict2022a}
\end{proposition}
}
