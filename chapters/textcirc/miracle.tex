\section{Internal wirings redux}
...
\marginnote{
\begin{defn}[Strictified string diagrams]
...
We are seeking some way to algebraically group or bracket together pregroup types that arise from a single word, in a distinguished way from concatenation-as-tensor. In this way we can preserve the structure of pregroup-sequent proofs: grouping indicates a node in the proof-tree, while tensor indicates parallel composition of proof trees. With strictified diagrams, we can model grouping (ab)using biased tensor structure, i.e. treating $(\cdots((A \otimes B) \otimes C) \cdots \otimes \cdots Z)$ as a bracketed expression $[A \otimes B \cdots \otimes Z]$.

\begin{construction}[Pregroups with bracketing]\label{cons:bracketing}
Where $\mathcal{P}$ is a monoidal category generated by pregroup states and (directed) cups, we define pregroups-with-bracketing as subcategory of the \emph{free} strictification $\overline{\mathcal{P}}$, generated by:
\begin{itemize}
...
\marginnote{
Cofunctors are arguably necessary even for vanilla DisCoCat. Pregroups by themselves are thin rigid autonomous categories, meaning there is at most one morphism between any pair of objects and that there are directed cups and caps in addition to plain monoidal structure. Contra the aims of DisCoCat, this implies there is no nontrivial strong monoidal functor from pregroups into $\textbf{FinVect}^\otimes$, where strength for a monoidal functor means that $Fx \otimes Fy \simeq F(x \otimes y)$ in the codomain. This is because for any object $x$ in the pregroup, any strong monoidal $F$ into $\textbf{FinVect}^\otimes$ that interprets duals in the usual way must identify the following diagrams up to isomorphism of finite vector spaces with their duals:
\[placeholder\]
The only vector space $Fx$ for which this holds is the base field of $\textbf{Vect}$; the monoidal unit. The same argument applies for functors into other strong compact closed categories, such as variants of $\textbf{Rel}$ -- another common family of semantic domains -- where the only valid target is the singleton as monoidal unit. 
}

Asking for opfibrations to be discrete is an unnecessarily strict condition. Often rather than asking for uniqueness, we are happy to supply a choice of lift...

\begin{defn}[Bijective-on-objects functor]
A functor $F: \mathcal{C} \rightarrow \mathcal{D}$ is \emph{bijective-on-objects} if for all $d \in \mathcal{D}$, there exists a $c \in \mathcal{C}$ such that $Fc = d$.
\end{defn}
